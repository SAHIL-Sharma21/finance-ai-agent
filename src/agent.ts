...
export async function processAgentMessage(message: any[]) {
  while (true) {
    const completion = await groq.chat.completions.create({
      messages: message,
      model: "openai/gpt-oss-20b",
...
function getToolsDefination(): ChatCompletionTool[] {
  return [
    {
      type: "function",        
      function: {              
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            name: {
              type: "string",
              description: "Name of the expense. e.g., Bought a macbook.",
            },
            amount: {
              type: "string",
    ...
    },
    {
      ...
      function: {
        ...
        description: "Get remaining money balance from your database.",
      }
    }
  ];
}
